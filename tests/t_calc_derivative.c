...
{
	int	size = 9;
	double	phs[] = {2.0, 3.0, 4.0, 4.4, 4.6, 6, 6.8, 8.0, 10.0, 11.3,
		11.46, 11.6, 11.8};
	double	volumes[] = {1.0, 2.0, 3.0, 5.0, 6.0, 7.0, 7.5, 8.0, 9.0,
		12.0, 14.0, 16.0, 20.0};
	double	results[] = {0.266667, -0.111111, -0.633333, -0.8, -0.666667,
		0.783333, 0.460333, 0.15, 0.0588333};
	double	derivative_2 = 0.0;
	int	i = 0;

	while (i < size) {
		derivative_2 = eqpt_derivative_old(2, &phs[i], &volumes[i]);
		cr_assert_eq((int) derivative_2, (int)results[i],
...
{
	FILE	*fd = NULL;
	double	results[] = {1.0, 0.47, 0.20, 0.8, 1.47, 2.0, 2.13, 0.83,
		0.29, 0.07, 0.06};
	int	size = 11;
	int	i = 0;
...
Test(calc_sec_derivative_ll, test1)
{
	...
	while (eqpt.deriv_head[1]->n->n != NULL) {
		cr_assert_eq((int) eqpt.deriv_head[1]->ph, (int) results[i],
		"got %g", eqpt.deriv_head[1]->vol);
